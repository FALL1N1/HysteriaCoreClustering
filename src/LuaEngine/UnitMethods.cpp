...
int LuaUnit::CanStartAttack(lua_State* L, Unit* unit)
{
    ...
    sEluna->PushBoolean(L, creature->CanStartAttack(target));
    return 1;
}
...
int LuaUnit::LeaveBattleground(lua_State* L, Unit* unit)
{
    ...
    player->LeaveBattleground(player->GetBattleground());
    return 0;
}
...
int LuaUnit::SummonPet(lua_State* L, Unit* unit)
{
    ...
    player->SummonPet(entry, x, y, z, o, (PetType)petType, despwtime, 0, player->GetGUID(), 0);
    return 0;
}
...
int LuaUnit::DealDamage(lua_State* L, Unit* unit)
{
    ...
    if (!target)
        unit->DealDamage(unit, 0, amount);
    else
        unit->DealDamage(unit, target, amount);
    return 0;
}
...
int LuaUnit::IsHorde(lua_State* L, Unit* unit)
{
    ...
    sEluna->PushBoolean(L, (player->GetTeamId() == TEAM_HORDE));
    return 1;
}
...
int LuaUnit::IsAlliance(lua_State* L, Unit* unit)
{
    ...
    sEluna->PushBoolean(L, (player->GetTeamId() == TEAM_ALLIANCE));
    return 1;
}
...
int LuaUnit::SendUnitWhisper(lua_State* L, Unit* unit)
{
    ...
    if (receiver && std::string(msg).length() > 0)
        if (Player* plr = receiver->ToPlayer())
        unit->MonsterWhisper(msg, plr, bossWhisper);
