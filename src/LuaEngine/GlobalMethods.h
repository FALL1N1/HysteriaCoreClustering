...
namespace LuaGlobalFunctions
{
    ...
    static int PerformIngameSpawn(lua_State* L)
    {
        ...
        if (spawntype == 1) // spawn creature
        {
            if (save)
            {
                ...
                if (!creature->Create(sObjectMgr->GenerateLowGuid(HIGHGUID_UNIT), map, phase, entry, 0, 0, x, y, z))
                {
                    delete creature;
                    return 0;
                }
        ...
        if (spawntype == 2) // Spawn object
        {
            ...
            if (!object->Create(lowguid, objectInfo->entry, map, phase, x, y, z, o, rotation, 0.0f, GO_STATE_READY, GO_STATE_READY))
            {
                delete object;
                return 0;
            }
    ...
    static int GetItemLink(lua_State* L)
    {
        ...
        */
        uint32 entry = luaL_checkunsigned(L, 1);
        LocaleConstant loc_idx = DEFAULT_LOCALE; // luaL_optint(L, 2, DEFAULT_LOCALE);
