...
private:
    ...
    public:
        ...
        void SendText(const Guild* guild, ClientSession* session) const;
...
public:
    static void SendCommandResult(ClientSession* session, GuildCommandType type, GuildCommandError errCode, std::string const& param = "");
    static void SendSaveEmblemResult(ClientSession* session, GuildEmblemError errCode);
    ...
    void HandleRoster(ClientSession* session);
    void HandleQuery(ClientSession* session);
    void HandleSetMOTD(ClientSession* session, std::string const& motd);
    void HandleSetInfo(ClientSession* session, std::string const& info);
    void HandleSetEmblem(ClientSession* session, const EmblemInfo& emblemInfo);
    void HandleSetLeader(ClientSession* session, std::string const& name);
    void HandleSetBankTabInfo(ClientSession* session, uint8 tabId, std::string const& name, std::string const& icon);
    void HandleSetMemberNote(ClientSession* session, std::string const& name, std::string const& note, bool officer);
    void HandleSetRankInfo(ClientSession* session, uint8 rankId, std::string const& name, uint32 rights, uint32 moneyPerDay, GuildBankRightsAndSlotsVec rightsAndSlots);
    void HandleBuyBankTab(ClientSession* session, uint8 tabId);
    void HandleInviteMember(ClientSession* session, std::string const& name);
    void HandleAcceptMember(ClientSession* session);
    void HandleLeaveMember(ClientSession* session);
    void HandleRemoveMember(ClientSession* session, std::string const& name);
    void HandleUpdateMemberRank(ClientSession* session, std::string const& name, bool demote);
    void HandleAddNewRank(ClientSession* session, std::string const& name);
    void HandleRemoveRank(ClientSession* session, uint8 rankId);
    void HandleRemoveLowestRank(ClientSession* session);
    void HandleMemberDepositMoney(ClientSession* session, uint32 amount);
    bool HandleMemberWithdrawMoney(ClientSession* session, uint32 amount, bool repair = false);
    void HandleMemberLogout(ClientSession* session);
    void HandleDisband(ClientSession* session);

    void UpdateMemberData(Player* player, uint8 dataid, uint32 value);
    void OnPlayerStatusChange(Player* player, uint32 flag, bool state);

    // Send info to client
    void SendInfo(ClientSession* session) const;
    void SendEventLog(ClientSession* session) const;
    void SendBankLog(ClientSession* session, uint8 tabId) const;
    void SendBankTabsInfo(ClientSession* session, bool showTabs = false) const;
    void SendBankTabData(ClientSession* session, uint8 tabId) const;
    void SendBankTabText(ClientSession* session, uint8 tabId) const;
    void SendPermissions(ClientSession* session) const;
    void SendMoneyInfo(ClientSession* session) const;
    void SendLoginInfo(ClientSession* session);
    ...
    // Broadcasts
    void BroadcastToGuild(ClientSession* session, bool officerOnly, std::string const& msg, uint32 language = LANG_UNIVERSAL) const;
    void BroadcastPacketToRank(WorldPacket* packet, uint8 rankId) const;
    void BroadcastPacket(WorldPacket* packet) const;

    void MassInviteToEvent(ClientSession* session, uint32 minLevel, uint32 maxLevel, uint32 minRank);
...
private:
    ...
    inline bool _HasRankRight(uint64 guid, uint32 right) const
    {
    ...
    void _SendBankContent(ClientSession* session, uint8 tabId) const;
    void _SendBankMoneyUpdate(ClientSession* session) const;
    void _SendBankContentUpdate(MoveItemData* pSrc, MoveItemData* pDest) const;
    void _SendBankContentUpdate(uint8 tabId, SlotIds slots) const;
    void _SendBankList(ClientSession* session = NULL, uint8 tabId = 0, bool sendFullSlots = false, SlotIds *slots = NULL) const;
