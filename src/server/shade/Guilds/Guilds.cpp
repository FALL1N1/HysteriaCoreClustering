...
void Guild::SendCommandResult(ClientSession* session, GuildCommandType type, GuildCommandError errCode, std::string const& param)
{
    ...
    ;//sLog->outDebug(LOG_FILTER_GUILD, "SMSG_GUILD_COMMAND_RESULT [%s]: Type: %u, code: %u, param: %s"
     //     , session->GetName(), type, errCode, param.c_str());
}

void Guild::SendSaveEmblemResult(ClientSession* session, GuildEmblemError errCode)
{
    WorldPacket data(MSG_SAVE_GUILD_EMBLEM, 4);
    data << uint32(errCode);
    session->SendPacket(&data);

    ;//sLog->outDebug(LOG_FILTER_GUILD, "MSG_SAVE_GUILD_EMBLEM [%s] Code: %u", session->GetName(), errCode);
}
...
void Guild::BankTab::SendText(Guild const* guild, ClientSession* session) const
{
    ...
    if (session)
    {
        ;//sLog->outDebug(LOG_FILTER_GUILD, "MSG_QUERY_GUILD_BANK_TEXT [%s]: Tabid: %u, Text: %s"
         //    , session->GetName(), m_tabId, m_text.c_str());
        session->SendPacket(&data);
    }
    else
    {
...
void Guild::Member::ChangeRank(uint8 newRank)
{
    ...
    if (Player* player = sObjectMgr->FindPlayer(m_guid))
...
bool Guild::Member::LoadFromDB(Field* fields)
{
    ...
    if (!m_zoneId)
    {
        ...
        m_zoneId = 1; // @guild Player::GetZoneIdFromDB(m_guid);
    }
    ResetFlags();
    return true;
}
...
bool Guild::Create(Player* pLeader, std::string const& name)
{
    ...
    ClientSession* pLeaderSession = pLeader->GetSession();
    if (!pLeaderSession)
        return false;
...
void Guild::HandleRoster(ClientSession* session)
{
    // Guess size
    ...
    for (Members::const_iterator itr = m_members.begin(); itr != m_members.end(); ++itr)
        itr->second->WritePacket(data, _HasRankRight(session->GetPlayer()->GetGUID(), GR_RIGHT_VIEWOFFNOTE));


    ;//sLog->outDebug(LOG_FILTER_GUILD, "SMSG_GUILD_ROSTER [%s]", session->GetName());
    session->SendPacket(&data);
}

void Guild::HandleQuery(ClientSession* session)
{
    ...
    session->SendPacket(&data);
    ;//sLog->outDebug(LOG_FILTER_GUILD, "SMSG_GUILD_QUERY_RESPONSE [%s]", session->GetName());
}

void Guild::HandleSetMOTD(ClientSession* session, std::string const& motd)
{
    if (m_motd == motd)
        return;
...
void Guild::HandleSetInfo(ClientSession* session, std::string const& info)
{
    if (m_info == info)
        return;
...
void Guild::HandleSetEmblem(ClientSession* session, const EmblemInfo& emblemInfo)
{ 
}

void Guild::HandleSetLeader(ClientSession* session, std::string const& name)
{
...
void Guild::HandleSetBankTabInfo(ClientSession* session, uint8 tabId, std::string const& name, std::string const& icon)
{
    BankTab* tab = GetBankTab(tabId);
    if (!tab)
    {
        //sLog->outError("Guild::HandleSetBankTabInfo: Player %s trying to change bank tab info from unexisting tab %d.", session->GetName(), tabId);
        return;
    }
...
void Guild::HandleSetMemberNote(ClientSession* session, std::string const& name, std::string const& note, bool isPublic)
{
...
void Guild::HandleSetRankInfo(ClientSession* session, uint8 rankId, std::string const& name, uint32 rights, uint32 moneyPerDay, GuildBankRightsAndSlotsVec rightsAndSlots)
{
...
void Guild::HandleBuyBankTab(ClientSession* session, uint8 tabId)
{ 
}

void Guild::HandleInviteMember(ClientSession* session, std::string const& name)
{
    Player* pInvitee = sObjectMgr->FindPlayerByName(name);
    if (!pInvitee)
    {
        sLog->outString("NO PLR FOUND");
...
void Guild::HandleAcceptMember(ClientSession* session)
{
    Player* player = session->GetPlayer();
...
void Guild::HandleLeaveMember(ClientSession* session)
{
    Player* player = session->GetPlayer();
    bool disband = false;
    ...
    // @guild sCalendarMgr->RemovePlayerGuildEventsAndSignups(player->GetGUID(), GetId());

    if (disband)
        delete this;
}

void Guild::HandleRemoveMember(ClientSession* session, std::string const& name)
{
    Player* player = session->GetPlayer();
...
void Guild::HandleUpdateMemberRank(ClientSession* session, std::string const& name, bool demote)
{
    Player* player = session->GetPlayer();
...
void Guild::HandleAddNewRank(ClientSession* session, std::string const& name)
{
...
void Guild::HandleRemoveLowestRank(ClientSession* session)
{
    HandleRemoveRank(session, _GetLowestRankId());
}

void Guild::HandleRemoveRank(ClientSession* session, uint8 rankId)
{
...
void Guild::HandleMemberDepositMoney(ClientSession* session, uint32 amount)
{
  
}

bool Guild::HandleMemberWithdrawMoney(ClientSession* session, uint32 amount, bool repair)
{
  
    return true;
}

void Guild::HandleMemberLogout(ClientSession* session)
{
    Player* player = session->GetPlayer();
    if (Member* member = GetMember(player->GetGUID()))
    {
...
void Guild::HandleDisband(ClientSession* session)
{
...
// Send data to client
void Guild::SendInfo(ClientSession* session) const
{
    ...
    session->SendPacket(&data);
    ;//sLog->outDebug(LOG_FILTER_GUILD, "SMSG_GUILD_INFO [%s]", session->GetName());
}

void Guild::SendEventLog(ClientSession* session) const
{
    WorldPacket data(MSG_GUILD_EVENT_LOG_QUERY, 1 + m_eventLog->GetSize() * (1 + 8 + 4));
    m_eventLog->WritePacket(data);
    session->SendPacket(&data);
    ;//sLog->outDebug(LOG_FILTER_GUILD, "MSG_GUILD_EVENT_LOG_QUERY [%s]", session->GetName());
}

void Guild::SendBankLog(ClientSession* session, uint8 tabId) const
{
    ...
    if (tabId < _GetPurchasedTabsSize() || tabId == GUILD_BANK_MAX_TABS)
    {
        ...
        session->SendPacket(&data);
        ;//sLog->outDebug(LOG_FILTER_GUILD, "MSG_GUILD_BANK_LOG_QUERY [%s]", session->GetName());
    }
}

void Guild::SendBankTabData(ClientSession* session, uint8 tabId) const
{
    if (tabId < _GetPurchasedTabsSize())
        _SendBankContent(session, tabId);
}

void Guild::SendBankTabsInfo(ClientSession* session, bool sendAllSlots /*= false*/) const
{
    _SendBankList(session, 0, sendAllSlots);
}

void Guild::SendBankTabText(ClientSession* session, uint8 tabId) const
{
    if (BankTab const* tab = GetBankTab(tabId))
        tab->SendText(this, session);
}

void Guild::SendPermissions(ClientSession* session) const
{
...
void Guild::SendMoneyInfo(ClientSession* session) const
{
    Member const* member = GetMember(session->GetPlayer()->GetGUID());
    if (!member)
        return;
    ...
    data << int32(amount);
    session->SendPacket(&data);
    ;//sLog->outDebug(LOG_FILTER_GUILD, "MSG_GUILD_BANK_MONEY_WITHDRAWN [%s] Money: %u", session->GetName(), amount);
}

void Guild::SendLoginInfo(ClientSession* session)
{
...
bool Guild::LoadMemberFromDB(Field* fields)
{
    ...
    sLogon->UpdateGlobalPlayerGuild(lowguid, GetId());
    return true;
}
...
// Broadcasts
void Guild::BroadcastToGuild(ClientSession* session, bool officerOnly, std::string const& msg, uint32 language) const
{
    ...
    {
        WorldPacket data;
        ChatHandler::BuildChatPacket(data, officerOnly ? CHAT_MSG_OFFICER : CHAT_MSG_GUILD, Language(language), session, NULL, msg);
        for (Members::const_iterator itr = m_members.begin(); itr != m_members.end(); ++itr)
            if (Player* player = sObjectMgr->FindPlayer(itr->second->GetGUID()))
                if (_HasRankRight(player->GetGUID(), officerOnly ? GR_RIGHT_OFFCHATLISTEN : GR_RIGHT_GCHATLISTEN) && !player->GetSocial()->HasIgnore(session->GetPlayer()->GetGUIDLow()))
...
void Guild::MassInviteToEvent(ClientSession* session, uint32 minLevel, uint32 maxLevel, uint32 minRank)
{
    return;
    /*
    uint32 count = 0;
...
{
    Player* player = sObjectMgr->FindPlayer(guid);
    ...
    else
    {
        ...
        m_members[lowguid] = member;
        sLogon->UpdateGlobalPlayerGuild(lowguid, m_id);
    }
...
{
    uint32 lowguid = GUID_LOPART(guid);
    Player* player = sObjectMgr->FindPlayer(guid); // @guild
    ...
    if (m_leaderGuid == guid && !isDisbanding)
    {
        ...
        if (Player* newLeaderPlayer = sObjectMgr->FindPlayer(newLeader->GetGUID()))
    ...
    if (player)
    {
        ...
        sLogon->UpdateGlobalPlayerGuild(lowguid, 0);
...
{
    if ((slotId >= GUILD_BANK_MAX_SLOTS && slotId != 255/* null slot*/) || tabId >= _GetPurchasedTabsSize())
        return;
...
{
    m_eventLog = new LogHolder(m_id, GUILD_EVENTLOG_MAX_RECORDS);
    for (uint8 tabId = 0; tabId <= GUILD_BANK_MAX_TABS; ++tabId)
        m_bankEventLog[tabId] = new LogHolder(m_id, GUILD_BANKLOG_MAX_RECORDS);
}
...
void Guild::_SendBankContent(ClientSession* session, uint8 tabId) const
{
...
void Guild::_SendBankMoneyUpdate(ClientSession* session) const
{
...
void Guild::_SendBankList(ClientSession* session /* = NULL*/, uint8 tabId /*= 0*/, bool sendAllSlots /*= false*/, SlotIds *slots /*= NULL*/) const
{
    ...
    if (session)
    {
        ...
        ;//sLog->outDebug(LOG_FILTER_GUILD, "SMSG_GUILD_BANK_LIST [%s]: TabId: %u, FullSlots: %u, slots: %d",
         //               session->GetName(), tabId, sendAllSlots, numSlots);
    }
    else // TODO - Probably this is just sent to session + those that have sent CMSG_GUILD_BANKER_ACTIVATE
    {
        for (Members::const_iterator itr = m_members.begin(); itr != m_members.end(); ++itr)
        {
            if (!_MemberHasTabRights(itr->second->GetGUID(), tabId, GUILD_BANK_RIGHT_VIEW_TAB))
                continue;
            Player* player = sObjectMgr->FindPlayer(itr->second->GetGUID());
            if (!player)
                continue;
