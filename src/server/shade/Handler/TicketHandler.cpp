...
void ClientSession::HandleGMTicketCreateOpcode(WorldPacket & recv_data)
{
    ...
    if (pPlayer->getLevel() < sLogon->getIntConfig(CONFIG_GMTICKET_LEVEL_REQ))
    {
        SendNotification(GetTrinityString(LANG_TICKET_REQ), sLogon->getIntConfig(CONFIG_GMTICKET_LEVEL_REQ));
        return;
    }

    GMTicketResponse response = GMTICKET_RESPONSE_CREATE_ERROR;
    // Player must not have ticket
    if (!sTicketMgr->GetTicketByPlayer(pPlayer->GetGUID()))
    {
        GmTicket* ticket = new GmTicket(pPlayer, recv_data);
        sTicketMgr->AddTicket(ticket);
        sTicketMgr->UpdateLastChange();

        sClientSessionMgr->SendGameMasterMessage(GetTrinityString(LANG_COMMAND_TICKETNEW), pPlayer->GetPlayerLink().c_str(), ticket->GetId());
...
void ClientSession::HandleGMTicketUpdateOpcode(WorldPacket & recv_data)
{
    std::string message;
    recv_data >> message;
    ...
    if (GmTicket* ticket = sTicketMgr->GetTicketByPlayer(GetPlayer()->GetGUID()))
    {
        ...
        sClientSessionMgr->SendGameMasterMessage(GetTrinityString(LANG_COMMAND_TICKETUPDATED), GetPlayer()->GetPlayerLink().c_str(), ticket->GetId());
...
void ClientSession::HandleGMTicketDeleteOpcode(WorldPacket & /*recv_data*/)
{
    if (GmTicket* ticket = sTicketMgr->GetTicketByPlayer(GetPlayer()->GetGUID()))
    {
        ...
        sClientSessionMgr->SendGameMasterMessage(GetTrinityString(LANG_COMMAND_TICKETPLAYERABANDON), GetPlayer()->GetPlayerLink().c_str(), ticket->GetId());
...
void ClientSession::HandleGMTicketGetTicketOpcode(WorldPacket & /*recv_data*/)
{
    // Node should response CMSG_QUERY_TIME
    SendPacketToNode(new WorldPacket(CMSG_QUERY_TIME));
...
void ClientSession::HandleGMTicketSystemStatusOpcode(WorldPacket & /*recv_data*/)
{
...
void ClientSession::HandleGMSurveySubmit(WorldPacket& recv_data)
{
...
void ClientSession::HandleReportLag(WorldPacket& recv_data)
{
...
void ClientSession::HandleGMResponseResolve(WorldPacket& /*recvPacket*/)
{
    // empty packet
    if (GmTicket* ticket = sTicketMgr->GetTicketByPlayer(GetPlayer()->GetGUID()))
    {
        uint8 getSurvey = 0;
        if (float(rand_chance()) < sLogon->getFloatConfig(CONFIG_GMTICKET_CHANCE_OF_GM_SURVEY))
            getSurvey = 1;
