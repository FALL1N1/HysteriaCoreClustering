...
public:
    ...
    Player* GetPlayer() const { return sObjectMgr->FindPlayer(_playerGuid); }
    uint64 GetPlayerGUID() const{ return _playerGuid; }
    std::string GetPlayerName() const { return _playerName; }
    std::string GetMessage() const { return _message; }
    Player* GetAssignedPlayer() const { return sObjectMgr->FindPlayer(_assignedTo); }
    ...
    uint64 GetClosedByGUID() const
    {
        if (IS_PLAYER_GUID(_closedBy))
            return _closedBy;
        else
            return 0;
    }
    ...
    void SetAssignedTo(uint64 guid, bool isAdmin)
    {
        ...
        sGMQualityManager->OnTicketAssign(this);
    }
    ...
    void SendResponse(ClientSession* session) const;
...
public:
    ...
    void SendTicket(ClientSession* session, GmTicket* ticket) const;
