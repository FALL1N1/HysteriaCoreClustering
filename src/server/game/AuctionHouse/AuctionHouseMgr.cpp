...
uint32 AuctionHouseMgr::GetAuctionDeposit(AuctionHouseEntry const* entry, uint32 time, Item* pItem, uint32 count)
{
    ...
    sLog->outChar("[PB Auction Logs] Player %s deposited>>> Entry: %u, MSV: %u, count: %u, multiplier: %f, deposit: %u ", pItem->GetOwner()->GetName().c_str(), pItem->GetEntry(), MSV, count, multiplier, deposit);
...
bool AuctionEntry::LoadFromDB(Field* fields)
{
    ...
    auctionHouseEntry = AuctionHouseMgr::GetAuctionHouseEntry(houseId);
    if (!auctionHouseEntry)
    {
        //sLog->outError("Auction %u has auctioneer (GUID : %u Entry: %u) with wrong houseId %u", Id, auctioneer, auctioneerData->id, houseId);
        return false;
    }
