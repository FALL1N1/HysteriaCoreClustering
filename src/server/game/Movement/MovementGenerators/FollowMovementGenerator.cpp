...
{
    if (owner->GetExactDistSq(target) > sqr(owner->GetCombatReach() + target->GetCombatReach() + range))
        return false;
...
bool FollowMovementGenerator::Update(Unit* owner, uint32 diff)
{
    ...
    if (owner->HasUnitState(UNIT_STATE_FOLLOW_MOVE))
    {
        ...
        else
        {
            ...
            if (PositionOkay(owner, target, -4.0f, _angle))
            {
                _path = nullptr;
                owner->StopMoving();
                return true;
    ...
    if (_lastTargetPosition.GetExactDistSq(target->GetPositionX(), target->GetPositionY(), target->GetPositionZ()) > 0.0f)
    {
        ...
        if (owner->HasUnitState(UNIT_STATE_FOLLOW_MOVE) || !PositionOkay(owner, target, -4.0f))
        {
            if (!_path)
            ...
            target->GetNearPoint(owner, x, y, z, -4.0f, 0, M_PI);//target->ToAbsoluteAngle(tAngle));
...
void FollowMovementGenerator::UpdatePetSpeed(Unit* owner)
{
    if (Pet* oPet = owner->ToPet())
        ...
        {
            oPet->UpdateSpeed(MOVE_RUN, true);
            oPet->UpdateSpeed(MOVE_WALK, true);
            oPet->UpdateSpeed(MOVE_SWIM, true);
        }
}
