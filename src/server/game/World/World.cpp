...
void World::LoadConfigSettings(bool reload)
{
    ...
    m_bool_configs[CONFIG_UNIQUE_GM] = sConfigMgr->GetBoolDefault("GM.Unique", true);
    ...
    m_bool_configs[CONFIG_API_DB_ENABLED] = sConfigMgr->GetBoolDefault("APIDatabase.Enabled", true);

    m_bool_configs[CONFIG_ARENA_QUEUE_ANNOUNCER_ENABLE] = sConfigMgr->GetBoolDefault("Custom.ArenaAnnounce", true);
    m_bool_configs[CONFIG_BATTLEGROUND_QUEUE_ANNOUNCER_PLAYERONLY] = sConfigMgr->GetBoolDefault("Custom.BGAnnounce.PlayerOnly", true);
    
    m_bool_configs[CONFIG_DUEL_CD_RESET] = sConfigMgr->GetBoolDefault("Custom.DuelCDReset", true);
    m_bool_configs[CONFIG_ACCOUNT_WIDE_MOUNTS] = sConfigMgr->GetBoolDefault("Custom.AccountWideMounts", true);
    m_bool_configs[CONFIG_ARENA_ANTI_DRAW] = sConfigMgr->GetBoolDefault("Custom.ArenaAntiDraw", true);
    m_bool_configs[CONFIG_DUEL_PHASING] = sConfigMgr->GetBoolDefault("Custom.DuelPhasing", false);
    ...
    m_bool_configs[CONFIG_OPTIONAL_TRANSPORT_GRID_PRELOAD] = sConfigMgr->GetBoolDefault("OptionalTransportGridPreload.Enabled", false);
    m_bool_configs[CONFIG_API_LOGGING_ENABLED] = sConfigMgr->GetBoolDefault("APIDatabase.Enabled", true);
    ...
    //! bools
    m_bool_configs[PREMIUM_CUSTOM_NAMES] = sConfigMgr->GetBoolDefault("Premiums.CustomNames", true);
    m_bool_configs[PREMIUM_CUSTOM_CHATC] = sConfigMgr->GetBoolDefault("Premiums.CustomChat", true);
    m_bool_configs[PREMIUM_CUSTOM_FREE_TAXI_PATHS] = sConfigMgr->GetBoolDefault("Premiums.FreeTaxiPaths", true);
    m_bool_configs[PREMIUM_CUSTOM_FREE_TAXI] = sConfigMgr->GetBoolDefault("Premiums.FreeTaxi", true);
    ...
    m_bool_configs[CUSTOM_SCRIPTS_ENABLED_OR_NOT] = sConfigMgr->GetBoolDefault("AreCustomScripts.Enabled", false);
    //! ints
    ...
    m_int_configs[CONFIG_GAME_TYPE] = sConfigMgr->GetIntDefault("MaxWhoListReturns", 49);
    if (reload)
    {
    ...
    m_int_configs[CONFIG_INSTANCE_RESET_TIME_DAY] = sConfigMgr->GetIntDefault("Instance.ResetTimeDay", 3);
    ...
    m_int_configs[CONFIG_BATTLEGROUND_INVITATION_TYPE]               = sConfigMgr->GetIntDefault ("Battleground.InvitationType", 2);
    ...
    m_bool_configs[CONFIG_ARENA_QUEUE_ENABLE]                        = sConfigMgr->GetBoolDefault("Arena.Queue.Enable", false); 
    ...
    m_bool_configs[CONFIG_DISPLAY_TOTAL_WHO]        = sConfigMgr->GetBoolDefault("ShowTotalPlayers.WhoList", true);
    ...
    // chat logging
    m_bool_configs[CONFIG_CHATLOG_CHANNEL] = sConfigMgr->GetBoolDefault("ChatLogs.Channel", true);
    m_bool_configs[CONFIG_CHATLOG_WHISPER] = sConfigMgr->GetBoolDefault("ChatLogs.Whisper", true);
    m_bool_configs[CONFIG_CHATLOG_SYSCHAN] = sConfigMgr->GetBoolDefault("ChatLogs.SysChan", false);
    m_bool_configs[CONFIG_CHATLOG_PARTY] = sConfigMgr->GetBoolDefault("ChatLogs.Party", true);
    m_bool_configs[CONFIG_CHATLOG_RAID] = sConfigMgr->GetBoolDefault("ChatLogs.Raid", true);
    m_bool_configs[CONFIG_CHATLOG_GUILD] = sConfigMgr->GetBoolDefault("ChatLogs.Guild", true);
    m_bool_configs[CONFIG_CHATLOG_PUBLIC] = sConfigMgr->GetBoolDefault("ChatLogs.Public", true);
    m_bool_configs[CONFIG_CHATLOG_ADDON] = sConfigMgr->GetBoolDefault("ChatLogs.Addon", false);
    m_bool_configs[CONFIG_CHATLOG_BGROUND] = sConfigMgr->GetBoolDefault("ChatLogs.BattleGround", true);
    m_bool_configs[CONFIG_CHATLOG_CONSOLE] = sConfigMgr->GetBoolDefault("ChatLogs.ShowInConsole", true);
    ...
    // arena template npc gear config
    // 1 = pvp
    // 2 = pve
    // 3 = both on
    
    m_int_configs[CONFIG_TEMPLATE_NPC_GEAR] = sConfigMgr->GetIntDefault("TemplateNPC.Gear", 3);
...
void World::SetInitialWorldSettings()
{
  ...
  */
    ...
    sLog->outError("Node initialization finished in %u minutes %u seconds", (startupDuration / 60000), ((startupDuration % 60000) / 1000));
    sLog->outString("                                 _______________________________________      ");
    sLog->outString();

    switch (sWorld->getIntConfig(CONFIG_CORE_TYPE))
    {
        case NODE_TYPE_MASTER:
            sLog->outString("MASTER NODE IS ALIVE AND OPEN TO CONNECTIONS, NO ERRORS OCCURRED.");
            break;
        case NODE_TYPE_EASTERN_KINGDOMS:
            sLog->outString("EASTERN KINGDOMS NODE IS ALIVE AND OPEN TO CONNECTIONS, NO ERRORS OCCURRED.");
            break;
        case NODE_TYPE_KALIMDOR:
            sLog->outString("KALIMDOR NODE IS ALIVE AND OPEN TO CONNECTIONS, NO ERRORS OCCURRED.");
            break;
        case NODE_TYPE_OUTLAND:
            sLog->outString("OUTLAND NODE IS ALIVE AND OPEN TO CONNECTIONS, NO ERRORS OCCURRED.");
            break;
        case NODE_TYPE_NORTHREND:
            sLog->outString("NORTHREND NODE IS ALIVE AND OPEN TO CONNECTIONS, NO ERRORS OCCURRED.");
            break;
        case NODE_TYPE_PVE:
            sLog->outString("PVE(RAID/DUNGEON) NODE IS ALIVE AND OPEN TO CONNECTIONS, NO ERRORS OCCURRED.");
            break;
        case NODE_TYPE_PVP:
            sLog->outString("PVP(BG/ARENA) NODE IS ALIVE AND OPEN TO CONNECTIONS, NO ERRORS OCCURRED.");
            break;
    }

    sLog->outString();
    sLog->outString("Proxy server informed, accepting connections.");
}
...
void World::ResetDailyQuests()
{
    ...
    // reset passive ac daily reports
...
void World::UpdateGlobalPlayerGroup(uint32 guid, uint32 groupId)
{
    ...
    // send group update to proxyserver
    WorldPacket data(NODE_PLAYER_DATA);
    data << uint32(CL_PLR_GROUP_UPDATE_ID);
    data << uint32(guid);
    data << uint32(groupId);
    SendGlobalMessage(&data);
}
