...
void ObjectMgr::CheckCreatureTemplate(CreatureTemplate const* cInfo)
{
    ...
    if (cInfo->Modelid1)
    {
        ...
        if (!displayEntry)
        {
            sLog->outErrorDb("Creature (Entry: %u) lists non-existing Modelid1 id (%i), this can crash the client.", cInfo->Entry, cInfo->Modelid1);
    ...
    if (cInfo->Modelid2)
    {
        ...
        if (!displayEntry)
        {
            sLog->outErrorDb("Creature (Entry: %u) lists non-existing Modelid2 id (%i), this can crash the client.", cInfo->Entry, cInfo->Modelid2);
    ...
    if (cInfo->Modelid3)
    {
        ...
        if (!displayEntry)
        {
            sLog->outErrorDb("Creature (Entry: %u) lists non-existing Modelid3 id (%i), this can crash the client.", cInfo->Entry, cInfo->Modelid3);
    ...
    if (cInfo->Modelid4)
    {
        ...
        if (!displayEntry)
        {
            sLog->outErrorDb("Creature (Entry: %u) lists non-existing Modelid4 id (%i), this can crash the client.", cInfo->Entry, cInfo->Modelid4);
...
uint8 ObjectMgr::CheckPlayerName(bool isPremium, const std::string& name, bool create)
{
    if (isPremium)
...
bool ObjectMgr::IsValidCharterName(bool isPremium, const std::string& name)
{
    if (isPremium)
        return true;
...
bool ObjectMgr::LoadTrinityStrings(const char* table, int32 min_value, int32 max_value)
{
    ...
    do
    {
        ...
        if (entry == 0)
        {
            sLog->outError("Table `%s` contain reserved entry 0, ignored.", table);
            continue;
        }
        else if (entry < start_value || entry >= end_value)
        {
            sLog->outError("Table `%s` contain entry %i out of allowed range (%d - %d), ignored.", table, entry, min_value, max_value);
            continue;
        }
        ...
        {
            sLog->outError("Table `%s` contain data for already loaded entry  %i (from another table?), ignored.", table, entry);
            continue;
        }
...
{
    if (TrinityStringLocale const* ts = GetTrinityStringLocale(entry))
    {
        if (ts->Content.size() > size_t(locale_idx) && !ts->Content[locale_idx].empty())
            return ts->Content[locale_idx].c_str();
    ...
    do
    {
        ...
        for (uint32 j = 0; j < MAX_CREATURE_OUTFIT_DISPLAYS; ++j)
        {
            ...
            if (displayInfo > 0) // entry
            {
                ...
                {
                    sLog->outError(">> Creature entry %u in `creature_template_outfits` has invalid item entry %u", entry, uint32(displayInfo));
                    co.outfit[j] = 0;
                }
            }
            else // display
