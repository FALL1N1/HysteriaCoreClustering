...
class ObjectMgr
{
    ...
    public:
        ...
        CreatureData const* GetCreatureData(uint64 guid) const
        {
            CreatureDataContainer::const_iterator itr = _creatureDataStore.find(guid);
            if (itr == _creatureDataStore.end()) return NULL;
            return &itr->second;
        }
        CreatureData& NewOrExistCreatureData(uint64 guid) { return _creatureDataStore[guid]; }
        void DeleteCreatureData(uint64 guid);
        ...
        static uint8 CheckPlayerName(bool isPremium, std::string const& name, bool create = false);
        static PetNameInvalidReason CheckPetName(std::string const& name);
        static bool IsValidCharterName(bool isPremium, std::string const& name);
