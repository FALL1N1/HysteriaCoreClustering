...
void BattlegroundMgr::Update(uint32 diff)
{
    ...
    if (sWorld->getBoolConfig(CONFIG_ARENA_AUTO_DISTRIBUTE_POINTS) && sWorld->getIntConfig(CONFIG_CORE_TYPE) <= NODE_TYPE_MASTER)
    {
        if (sWorld->GetGameTime() > m_NextAutoDistributionTime)
        {
            sArenaTeamMgr->DistributeArenaPoints();
            m_NextAutoDistributionTime = sWorld->GetNextTimeWithDayAndHour(5, 18);
            sWorld->setWorldState(20001, uint64(m_NextAutoDistributionTime));
        }
    }
}
...
void BattlegroundMgr::InitAutomaticArenaPointDistribution()
{
    ...
    time_t wstime = time_t(sWorld->getWorldState(20001));
...
void BattlegroundMgr::InviteGroupToBG(GroupQueueInfo* ginfo, Battleground* bg, TeamId teamId)
{
    ...
    for (std::set<uint64>::iterator itr = ginfo->Players.begin(); itr != ginfo->Players.end(); ++itr)
    {
        ...
        // Set player has joined premade if more than 1 player
        if (increaseCountPremade)
        {
