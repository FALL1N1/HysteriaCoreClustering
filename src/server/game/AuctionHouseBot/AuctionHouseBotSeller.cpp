...
bool AuctionBotSeller::Initialize()
{
    ...
    // sLog->outString("Forced Inclusion %u items", (uint32)includeItems.size());
    // sLog->outString("Forced Exclusion %u items", (uint32)excludeItems.size());
    ...
    // sLog->outString("Npc vendor filter has %u items", (uint32)npcItems.size());
    ...
    // sLog->outString("Loot filter has %u items", (uint32)lootItems.size());
    ...
    if (!itemsAdded)
    {
        // sLog->outString("AuctionHouseBot seller not have items, disabled.");
    ...
    // sLog->outString("Items loaded \tGray\tWhite\tGreen\tBlue\tPurple\tOrange\tYellow");
    //for (uint32 i = 0; i < MAX_ITEM_CLASS; ++i)
        // sLog->outString("\t\t%u\t%u\t%u\t%u\t%u\t%u\t%u", (uint32)_itemPool[0][i].size(), (uint32)_itemPool[1][i].size(), (uint32)_itemPool[2][i].size(), (uint32)_itemPool[3][i].size(), (uint32)_itemPool[4][i].size(), (uint32)_itemPool[5][i].size(), (uint32)_itemPool[6][i].size());
...
void AuctionBotSeller::LoadSellerValues(SellerConfiguration& config)
{
    ...
    // sLog->outString("AHBot: GrayItems = %u", config.GetItemsAmountPerQuality(AUCTION_QUALITY_GRAY));
...
uint32 AuctionBotSeller::SetStat(SellerConfiguration& config)
{
    ...
    // sLog->outString("AHBot: Missed Item       \tGray\tWhite\tGreen\tBlue\tPurple\tOrange\tYellow");
    for (uint32 i = 0; i < MAX_ITEM_CLASS; ++i)
    {
        // sLog->outString("AHBot: \t\t%u\t%u\t%u\t%u\t%u\t%u\t%u",
...
void AuctionBotSeller::AddNewAuctions(SellerConfiguration& config)
{
    ...
    if (config.LastMissedItem > sAuctionBotConfig->GetItemPerCycleBoost())
    {
        ...
        // sLog->outString("AHBot: Boost value used to fill AH! (if this happens often adjust both ItemsPerCycle in worldserver.conf)");
    }
    else
    ...
    // sLog->outString("AHBot: Added %u items to auction", count);
}
...
bool AuctionBotSeller::Update(AuctionHouseType houseType)
{
    ...
    {
        // sLog->outString("AHBot: %s selling ...", AuctionBotConfig::GetHouseTypeName(houseType));
