/*
 * Copyright (C) 2008-2016 ProjectB <http://www.Balnazzar.org/>
 *
...
bool AuctionBotBuyer::Initialize()
{
    ...
    // sLog->outString("AHBot buyer interval is %u minutes", _checkInterval / MINUTE);
    return true;
}
...
uint32 AuctionBotBuyer::GetItemInformation(BuyerConfiguration& config)
{
    ...
    // sLog->outString("AHBot: %u items added to buyable/biddable vector for ah type: %u", count, config.GetHouseType());
    // sLog->outString("AHBot: SameItemInfo size = %u", (uint32)config.SameItemInfo.size());
    return count;
}
...
bool AuctionBotBuyer::RollBuyChance(const BuyerItemInfo* ahInfo, const Item* item, const AuctionEntry* auction, uint32 /*bidPrice*/)
{
    ...
    if (ahInfo)
    {
        ...
        // sLog->outString("AHBot: buyout average: %.1f items with buyout: %u", avgBuyPrice, ahInfo->BuyItemCount);
    ...
    // sLog->outString("AHBot: %s BUY! chance = %.2f, price = %u, buyprice = %u.", win ? "WIN" : "LOSE", chance, uint32(itemPrice), uint32(itemBuyPrice));
    return win;
}
...
bool AuctionBotBuyer::RollBidChance(const BuyerItemInfo* ahInfo, const Item* item, const AuctionEntry* auction, uint32 bidPrice)
{
    ...
    if (ahInfo)
    {
        ...
        // sLog->outString("AHBot: Bid average: %.1f biddable item count: %u", avgBidPrice, ahInfo->BidItemCount);
    ...
    float rand = frand(0.f, 100.f);
    bool win = rand <= chance;
    // sLog->outString("AHBot: %s BID! chance = %.2f, price = %u, bidprice = %u.", win ? "WIN" : "LOSE", chance, uint32(itemPrice), uint32(itemBidPrice));
    return win;
}
...
void AuctionBotBuyer::PrepareListOfEntry(BuyerConfiguration& config)
{
    ...
    // sLog->outString("AHBot: EligibleItems size = %u", (uint32)config.EligibleItems.size());
}
...
void AuctionBotBuyer::BuyAndBidItems(BuyerConfiguration& config)
{
    ...
    if (items.size() > sAuctionBotConfig->GetItemPerCycleBoost())
    {
        ...
        // sLog->outString("AHBot: Boost value used for Buyer! (if this happens often adjust both ItemsPerCycle in worldserver.conf)");
    }
    ...
    while (cycles && itr != items.end())
    {
        ...
        if (!auction)
        {
            // sLog->outString("AHBot: Entry %u doesn't exists, perhaps bought already?", itr->second.AuctionId);
            items.erase(itr++);
            continue;
        }
        ...
        const BuyerItemInfo* ahInfo = NULL;
        ...
        // sLog->outString("AHBot: Rolling for AHentry %u:", auction->Id);
...
{
    // sLog->outString("AHBot: Entry %u bought at %.2fg", auction->Id, float(auction->buyout) / GOLD);
...
{
    // sLog->outString("AHBot: Bid placed to entry %u, %.2fg", auction->Id, float(bidPrice) / GOLD);
