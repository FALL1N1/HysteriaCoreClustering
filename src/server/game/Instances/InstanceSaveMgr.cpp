...
InstanceSave* InstanceSaveManager::AddInstanceSave(uint32 mapId, uint32 instanceId, Difficulty difficulty, bool startup /*=false*/)
{
    ...
    time_t custom_hour, custom_day, custom_time;
    custom_hour = sWorld->getIntConfig(CONFIG_INSTANCE_RESET_TIME_HOUR) * HOUR;
    custom_day = sWorld->getIntConfig(CONFIG_INSTANCE_RESET_TIME_DAY) * DAY;
    custom_time = custom_day + custom_hour;
