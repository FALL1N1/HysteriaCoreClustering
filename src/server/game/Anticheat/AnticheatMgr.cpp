...
void AnticheatMgr::StartHackDetection(Player* player, MovementInfo movementInfo, uint32 opcode)
{
    ...
    if (player->IsGameMaster() && player->GetSession()->GetSecurity() >= 1)
        return;
...
void AnticheatMgr::ClimbHackDetection(Player *player, MovementInfo movementInfo, uint32 opcode)
{
    ...
    float deltaZ = fabs(player->GetPositionZ() - movementInfo.pos.GetPositionZ());
        ...
        if (action <= 2)
        {
            switch (action)
            {
                ...
                case BAN:
                    ...
                    sLog->outString("PASSIVEANTICHEAT: [Player:%s(Guid : %u, Account : %u)] Banned for exceeding maximum reports", player->GetName().c_str(), player->GetGUIDLow(), player->GetSession()->GetAccountId());
            ...
            sLog->outString("Invalid action specified for Anticheat.Action in the config");
    }
...
void AnticheatMgr::AnticheatGlobalCommand(ChatHandler* handler)
{
        ...
        do
        {
            ...
            if (guid > 0)
                if (sObjectMgr->GetPlayerNameByGUID(guid, name))
                    handler->PSendSysMessage("Player: %s Average: %f Total Reports: %u", name.c_str(), average, total_reports);
    ...
    } else
    {
        ...
        do
        {
            ...
            if (guid > 0)
                if (sObjectMgr->GetPlayerNameByGUID(guid, name))
                    handler->PSendSysMessage("Player: %s Total Reports: %u Average: %f", name.c_str(),total_reports,average);
