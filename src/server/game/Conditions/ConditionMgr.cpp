...
bool ConditionMgr::isConditionTypeValid(Condition* cond)
{
    ...
    switch (cond->ConditionType)
    {
        ...
        case CONDITION_GUILD_LEVEL:
        {
            if (cond->ConditionValue2 >= COMP_TYPE_MAX)
            {
                sLog->outErrorDb("Guildlevel condition has invalid option (%u), skipped", cond->ConditionValue2);
                return false;
            }
            if (cond->ConditionValue3)
                sLog->outErrorDb("Guildlevel condition has useless data in value3 (%u)!", cond->ConditionValue3);
            break;
        }
        default:
            break;
