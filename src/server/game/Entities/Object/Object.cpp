...
{ 
    ...
    if (oldval != newval)
    {
        ...
        {
            ToPlayer()->C_UpdateFlags(true, newval);
        }
...
{ 
    ...
    if (oldval != newval)
    {
        ...
        if (GetTypeId() == TYPEID_PLAYER)
        {
            ToPlayer()->C_UpdateFlags(false, newval);
        }
...
Position WorldObject::GetHitSpherePointFor(Position const& dest) const
{
    ...
    G3D::Vector3 contactPoint = vThis + (vObj - vThis).directionOrZero() * std::min(GetExactDist(&dest), GetObjectSize());
    Position tmp = { contactPoint.x, contactPoint.y, contactPoint.z, GetAngle(contactPoint.x, contactPoint.y) };
    return tmp;
}
...
void WorldObject::MovePositionToFirstCollisions(Position &pos, float dist, float angle) const
{
    angle += GetOrientation();
    float destx, desty, destz, ground, floor;
