...
void CrossFaction::DoForgetPlayersInBG(Battleground* pBattleGround, Player* player)
{
    ...
    {
        sLog->outString("player %s - Reset Cache", player->GetName().c_str());
...
void CrossFaction::SetFakeRaceAndMorph(Player* player)
{
    if (player->getClass() == CLASS_DRUID)
    {
        if (player->GetTeamId(true) == TEAM_ALLIANCE) // ALLT
        {
            ...
            sLog->outString("player %s is a tauren !", player->GetName().c_str());
        }
        else // HORDE
        {
            ...
            sLog->outString("player %s is a nightelf !", player->GetName().c_str());
        }         
    }
    else
    {
        ...
        {
            sLog->outString("player %s is a human !", player->GetName().c_str());
        ...
        {
            sLog->outString("player %s is a bloodelf now!", player->GetName().c_str());
...
void CrossFaction::UpdatePlayerTeam(Group* group, uint64 guid, bool reset /* = false */)
{
    ...
    if (player)
    {
        ...
        if (isMapEnabled(player->GetMapId()) && isZoneEnabled(player->GetZoneId()) && isAreaEnabled(player->GetAreaId()))
            ...
            sLog->outString("Crossfaction: disabled for player %s", player->GetName().c_str());
...
void CrossFaction::UpdateAllGroups()
{
    for (std::unordered_map<uint64, uint8>::iterator itr = LeaderRaceMap.begin(); itr != LeaderRaceMap.end(); itr++)
        if (Player* leader = ObjectAccessor::FindPlayer(itr->first))
            ...
            {
                sLog->outString("Updating faction for group of leader %s", leader->GetName().c_str());
...
public:
    ...
    void OnUpdateFaction(Player* player)
    {
        sCrossFaction->SetFakeRaceAndMorph(player); // set fake race information
        sCrossFaction->UpdatePlayerTeam(player->GetGroup(), player->GetGUID());
    }

    void OnAddToBattleground(Player* player, Battleground* bg)
    {
        if (player && bg)
        {
            sLog->outString("adding player %u to bg", player->GetGUID());
    ...
    void OnRemoveFromBattleground(Player* player, Battleground* bg)
    {
        if (player && bg)
        {
            sLog->outString("removing player %u from bg", player->GetGUID());
