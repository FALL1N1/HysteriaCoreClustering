...
void WorldSession::HandleCharCreateOpcode(WorldPacket& recvData)
{
    ...
    uint8 res = ObjectMgr::CheckPlayerName(IsPremium(), name, true);
...
//            chH.PSendSysMessage(_FULLVERSION);
    ...
    if (uint32 guildId = Player::GetGuildIdFromStorage(pCurrChar->GetGUIDLow()))
    {
        ...
        if (member)
        {
            ...
            pCurrChar->C_SendGuildId(guildId);
        }
        else
        {
    ...
    if (!pCurrChar->GetMap()->AddPlayerToMap(pCurrChar) || !pCurrChar->CheckInstanceLoginValid())
    {
        ...
        if (at)
            ...
            pCurrChar->TeleportTo(pCurrChar->m_homebindMapId, pCurrChar->m_homebindX, pCurrChar->m_homebindY, pCurrChar->m_homebindZ, pCurrChar->GetOrientation());
        sLog->outString("TELEPORT BUG HERE #1! ");
    }
    ...
    if (sWorld->getBoolConfig(CONFIG_ALL_TAXI_PATHS) || (pCurrChar->GetSession()->IsPremium() && sWorld->getBoolConfig(PREMIUM_CUSTOM_FREE_TAXI_PATHS)))
    ...
    std::string IP_str = GetRemoteAddress();
    //sLog->outChar("Account: %d (IP: %s) Login Character:[%s] (GUID: %u) Level: %d", GetAccountId(), IP_str.c_str(), pCurrChar->GetName().c_str(), pCurrChar->GetGUIDLow(), pCurrChar->getLevel());
    ...
    pCurrChar->PerformNodeChangeStuff("WorldSession::HandlePlayerLoginFromDB CharacterHandler.cpp:1138");
...
void WorldSession::HandleCharRenameOpcode(WorldPacket& recvData)
{
    ...
    uint8 res = ObjectMgr::CheckPlayerName(IsPremium(), newName, true);
    if (res != CHAR_NAME_SUCCESS)
    {
