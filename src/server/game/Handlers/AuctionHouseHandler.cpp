...
void WorldSession::HandleAuctionSellItem(WorldPacket & recvData)
{
    ...
    for (uint32 i = 0; i < itemsCount; ++i)
    {
        ...
        {
            CreatureData const* auctioneerData = sObjectMgr->GetCreatureData(creature->GetEntry());
            if (!auctioneerData)
            {
                sLog->outString("Data for auctioneer not found (%u)", auctionHouseEntry);
                return;
            }
            ...
            if (!auctioneerInfo)
            {
                sLog->outString("Non existing auctioneer (%u)", auctionHouseEntry);
                return;
            }
