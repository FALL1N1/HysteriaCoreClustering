...
class spell_onslaught_horse_deliver : public SpellScriptLoader
{
public:
    spell_onslaught_horse_deliver() : SpellScriptLoader("spell_onslaught_horse_deliver") { }

    class spell_onslaught_horse_deliver_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_onslaught_horse_deliver_SpellScript);

        void OnCast()
        {
            if (!GetCaster())
            {
                sLog->outString("[spell_generic.cpp:4864] Something is messed up with the spell, no caster ??? :(");
                return;
            }

            if (Creature* horse = GetCaster()->ToCreature())
                if (Player* plr = horse->GetCharmerOrOwner()->ToPlayer())
                {
                    // dismount or exit vehicle
                    plr->Dismount();
                    plr->ExitVehicle();

                    // kill and despawn horse? maybe removing veh flag and dissaperning would be better
                    //horse->
        ...
        void Register()
        {
            BeforeHit += SpellHitFn(spell_onslaught_horse_deliver_SpellScript::OnCast);
        }
    };

    SpellScript* GetSpellScript() const
    {
        return new spell_onslaught_horse_deliver_SpellScript();
    }
};
