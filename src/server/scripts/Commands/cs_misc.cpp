...
public:
    ...
    static bool HandleUnstuckCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (location_str == "inn")
        {
            ...
            sLog->outString("TELEPORT BUG HERE #11! ");
            return true;
        }
   ...
   static bool HandleFakePlayer(ChatHandler* handler, char const* args)
   {
    ...
    {
        std::string Handle = (char*)args;

        if (Handle == "on")
        {
            sWorld->setBoolConfig(CONFIG_ARENA_QUEUE_ENABLE, true);
            sWorld->SendWorldText(3, ".::Arena Queue Enabled::.");
        }
        else if (Handle == "off")
        {
            sWorld->setBoolConfig(CONFIG_ARENA_QUEUE_ENABLE, false);
            sWorld->SendWorldText(3, ".::Arena Queue Disabled::.");
        }
        else
            ChatHandler(handler->GetSession()).PSendSysMessage("Wrong!");
            return true;
    }
};
