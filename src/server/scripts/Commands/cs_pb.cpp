...
public:
    ...
    {
        static std::vector<ChatCommand> BalnazzarCommandTable =
        {
            { "damage",                             SEC_ADMINISTRATOR, false, &HandleBalnazzarDamage,       ""},
            { "maxlevel",                           SEC_ADMINISTRATOR, false, &HandleBalnazzarMaxLevel,     ""},
            { "minlevel",                           SEC_ADMINISTRATOR, false, &HandleBalnazzarMinLevel,     "" },
            { "teleport",                           SEC_ADMINISTRATOR, false, &HandleBalnazzarTeleport,     "" },
            { "godmode",                            SEC_ADMINISTRATOR, false, &HandleBalnazzarGodmode,      "" },
            { "learnspells",                        SEC_ADMINISTRATOR, false, &HandleBalnazzarLearnSpell,   "" },
            { "gold",                               SEC_ADMINISTRATOR, false, &HandleBalnazzarSetGold,      "" },
            { "nocooldown",                         SEC_ADMINISTRATOR, false, &HandleBalnazzarNoCD,         "" },
            { "nopower",                            SEC_ADMINISTRATOR, false, &HandleBalnazzarNoPOWER,      "" },
            { "maxskill",                           SEC_ADMINISTRATOR, false, &HandleBalnazzarMaxSkill,     "" },

        };

        static std::vector<ChatCommand> CommandTable =
        {
            { "test",            SEC_PLAYER,             true,  NULL,                                "", BalnazzarCommandTable }, 
        };
    ...
    static bool HandleBalnazzarNoPOWER(ChatHandler* handler, const char* args)
    {
        if (!handler->GetSession() && !handler->GetSession()->GetPlayer())
            return false;
    ...
    static bool HandleBalnazzarSetGold(ChatHandler* handler, const char* args)
    {
        Player* player = handler->GetSession()->GetPlayer();
        player->ModifyMoney(214000000);
        return true;
    }


    static bool HandleBalnazzarLearnSpell(ChatHandler* handler, const char* args)
    {
        Player* player = handler->GetSession()->GetPlayer();
    ...
    static bool HandleBalnazzarTeleport(ChatHandler* handler, const char* args)
    {
        if (!*args)
            return false;
    ...
    static bool HandleBalnazzarGodmode(ChatHandler* handler, const char* args)
    {
        if (!handler->GetSession() && !handler->GetSession()->GetPlayer())
            return false;
    ...
    static bool HandleBalnazzarDamage(ChatHandler* handler, char const* args)
    {
    ...
    static bool HandleBalnazzarMinLevel(ChatHandler* handler, char const* args)
    {
        if (Player* plr = handler->GetSession()->GetPlayer())
        {
            plr->SetLevel(1);
    ...
    static bool HandleBalnazzarMaxLevel(ChatHandler* handler, char const* args)
    {
        if (Player* plr = handler->GetSession()->GetPlayer())
        {
            plr->SetLevel(80);
