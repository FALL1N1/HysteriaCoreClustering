...
public:
    ...
    struct instance_naxxramas_InstanceMapScript : public InstanceScript
    {
        instance_naxxramas_InstanceMapScript(Map* pMap) : InstanceScript(pMap)
        {
            ...
            _maexxnaGateGUID = 0;
            _thaddiusGateGUID = 0;
            _thaddiusTeslaGUID = 0;
            _thaddiusTeslaTwoGUID = 0;
            _horsemanGateGUID = 0;
            ...
            _sapphironportalGUID = 0;
            _sapphironportaleffectGUID = 0;
            _plagueportaleffectGUID = 0;
            _dkportaleffect_GUID = 0;
            _abomportaleffectGUID = 0;
            _spidereffectGUID = 0;
            _horsemanPortalEffectGUID = 0;
            _loathebPortalEffectGUID = 0;
            _maexxnaPortalEffectGUID = 0;
            _thaddiusPortalEffectGUID = 0;
        ...
        uint64 _thaddiusTeslaGUID;
        uint64 _thaddiusTeslaTwoGUID;
        ...
        uint64 _sapphironportaleffectGUID;
        uint64 _plagueportaleffectGUID;
        uint64 _dkportaleffect_GUID;
        uint64 _abomportaleffectGUID;
        uint64 _spidereffectGUID;
        uint64 _horsemanPortalEffectGUID;
        uint64 _loathebPortalEffectGUID;
        uint64 _maexxnaPortalEffectGUID;
        uint64 _thaddiusPortalEffectGUID;
        ...
        void OnGameObjectCreate(GameObject* pGo)
        {
            ...
            case GO_SAPPHIRON_PORTAL:
                ...
                if (Encounters[EVENT_THADDIUS] && Encounters[EVENT_HORSEMAN] && Encounters[EVENT_MAEXXNA] && Encounters[EVENT_LOATHEB] == DONE)
                    pGo->SetPhaseMask(1, true);
                break;
            case GO_SAPPHIRON_EFFECT:
                _sapphironportaleffectGUID = pGo->GetGUID();
                if (Encounters[EVENT_THADDIUS] && Encounters[EVENT_HORSEMAN] && Encounters[EVENT_MAEXXNA] && Encounters[EVENT_LOATHEB] == DONE)
                    pGo->SetGoState(GO_STATE_ACTIVE);
                break;
            case GO_PLAGUE_PORTAL_EFFECT:
                _plagueportaleffectGUID = pGo->GetGUID();
                if (Encounters[EVENT_LOATHEB] == DONE)
                    pGo->SetGoState(GO_STATE_ACTIVE);
                break;
            case GO_DK_PORTAL_EFFECT:
                _dkportaleffect_GUID = pGo->GetGUID();
                if (Encounters[EVENT_HORSEMAN] == DONE)
                    pGo->SetGoState(GO_STATE_ACTIVE);
                break;
            case GO_ABOM_PORTAL_EFFECT:
                _abomportaleffectGUID = pGo->GetGUID();
                if (Encounters[EVENT_THADDIUS] == DONE)
                    pGo->SetGoState(GO_STATE_ACTIVE);
                break;
            case GO_SPIDER_PORTAL_EFFECT:
                _spidereffectGUID = pGo->GetGUID();
            ...
            case GO_NOTH_ENTRANCE_GATE:
                _nothGateEntranceGUID = pGo->GetGUID();
                if (Encounters[EVENT_NOTH] == IN_PROGRESS)
                    pGo->SetGoState(GO_STATE_ACTIVE);
                break;
            ...
            case GO_THADDIUS_GATE:
                ...
                if (Encounters[EVENT_GLUTH] == DONE) // gluth is correct
                    pGo->SetGoState(GO_STATE_ACTIVE);
                break;
            case GO_THADDIUS_TESLA:
                _thaddiusTeslaGUID = pGo->GetGUID();
                break;
            case GO_THADDIUS_TESLA_TWO:
                _thaddiusTeslaTwoGUID = pGo->GetGUID();
                break;
            ...
            case GO_DEATHKNIGHT_WING:
                ...
                if (Encounters[EVENT_LOATHEB] == DONE)
                    pGo->SetPhaseMask(1, true);
                break;
            case GO_DEATHKNIGHT_WING_EFFECT:
                _loathebPortalEffectGUID = pGo->GetGUID();
                if (Encounters[EVENT_LOATHEB] == DONE)
                    pGo->setActive(GO_STATE_ACTIVE);
                break;
            case GO_THADDIUS_PORTAL:
                ...
                if (Encounters[EVENT_THADDIUS] == DONE)
                    pGo->SetPhaseMask(1, true);
                break;
            case GO_THADDIUS_PORTAL_EFFECT:
                _thaddiusPortalEffectGUID = pGo->GetGUID();
                if (Encounters[EVENT_THADDIUS] == DONE)
                    pGo->setActive(GO_STATE_ACTIVE);
                break;
            case GO_MAEXXNA_PORTAL:
                ...
                if (Encounters[EVENT_MAEXXNA] == DONE)
                    pGo->SetPhaseMask(1, true);
                break;
            case GO_MAEXXNA_PORTAL_EFFECT:
                _maexxnaPortalEffectGUID = pGo->GetGUID();
                if (Encounters[EVENT_MAEXXNA] == DONE)
                    pGo->setActive(GO_STATE_ACTIVE);
                break;
            case GO_HORSEMAN_PORTAL:
                ...
                if (Encounters[EVENT_HORSEMAN] == DONE)
                    pGo->SetPhaseMask(1, true);
                break;
            case GO_HORSEMAN_PORTAL_EFFECT:
                _horsemanPortalEffectGUID = pGo->GetGUID();
                if (Encounters[EVENT_HORSEMAN] == DONE)
                    pGo->setActive(GO_STATE_ACTIVE);
                break;
            }
            ...
            if (data == DONE)
            {
                ...
                case EVENT_LOATHEB:
                    if (GameObject* go = instance->GetGameObject(_loathebGateGUID))
                        go->SetGoState(GO_STATE_ACTIVE);
                    if (GameObject* go = instance->GetGameObject(_loathebPortalEffectGUID))
                        go->SetGoState(GO_STATE_ACTIVE);
                    if (GameObject* go = instance->GetGameObject(_loathebPortalGUID))
                        go->SetPhaseMask(1, true);
                    if (GameObject* go = instance->GetGameObject(_plagueportaleffectGUID))
                        go->SetGoState(GO_STATE_ACTIVE);
                    if (Encounters[EVENT_HORSEMAN] && Encounters[EVENT_THADDIUS] && Encounters[EVENT_MAEXXNA] == DONE)
                    {
                        if (GameObject* go = instance->GetGameObject(_sapphironportalGUID))
                            go->SetPhaseMask(1, true);
                        if (GameObject* go = instance->GetGameObject(_sapphironportaleffectGUID))
                            go->SetGoState(GO_STATE_ACTIVE);
                    }
                    break;
                ...
                case EVENT_MAEXXNA:
                    if (GameObject* go = instance->GetGameObject(_maexxnaGateGUID))
                        go->SetGoState(GO_STATE_ACTIVE);
                    if (GameObject* go = instance->GetGameObject(_maexxnaPortalEffectGUID))
                        go->SetGoState(GO_STATE_ACTIVE);
                    if (GameObject* go = instance->GetGameObject(_maexxnaPortalGUID))
                        go->SetPhaseMask(1, true);
                    if (GameObject* go = instance->GetGameObject(_spidereffectGUID))
                        go->SetGoState(GO_STATE_ACTIVE);
                    if (Encounters[EVENT_HORSEMAN] && Encounters[EVENT_THADDIUS] && Encounters[EVENT_LOATHEB] == DONE)
                    {
                        if (GameObject* go = instance->GetGameObject(_sapphironportalGUID))
                            go->SetPhaseMask(1, true);
                        if (GameObject* go = instance->GetGameObject(_sapphironportaleffectGUID))
                ...
                case EVENT_THADDIUS:
                    if (GameObject* go = instance->GetGameObject(_thaddiusPortalGUID))
                        go->SetPhaseMask(1, true);
                    if (GameObject* go = instance->GetGameObject(_thaddiusPortalEffectGUID))
                        go->SetGoState(GO_STATE_ACTIVE);
                    if (GameObject* go = instance->GetGameObject(_abomportaleffectGUID))
                        go->SetGoState(GO_STATE_ACTIVE);
                    if (Encounters[EVENT_HORSEMAN] && Encounters[EVENT_MAEXXNA] && Encounters[EVENT_LOATHEB] == DONE)
                    {
                        if (GameObject* go = instance->GetGameObject(_sapphironportalGUID))
                            go->SetPhaseMask(1, true);
                        if (GameObject* go = instance->GetGameObject(_sapphironportaleffectGUID))
                ...
                case EVENT_HORSEMAN:
                    if (GameObject* go = instance->GetGameObject(_horsemanPortalGUID))
                        go->SetPhaseMask(1, true);
                    if (GameObject* go = instance->GetGameObject(_horsemanPortalEffectGUID))
                        go->SetGoState(GO_STATE_ACTIVE);
                    if (GameObject* go = instance->GetGameObject(_dkportaleffect_GUID))
                        go->SetGoState(GO_STATE_ACTIVE);
                    if (Encounters[EVENT_THADDIUS] && Encounters[EVENT_MAEXXNA] && Encounters[EVENT_LOATHEB] == DONE)
                    {
                        if (GameObject* go = instance->GetGameObject(_sapphironportalGUID))
                            go->SetPhaseMask(1, true);
                        if (GameObject* go = instance->GetGameObject(_sapphironportaleffectGUID))
        ...
        void Update(uint32 diff)
        {
            if (screamsTimer && Encounters[EVENT_THADDIUS] != DONE)
            {
                if(Encounters[EVENT_GLUTH] != DONE)
                {
                    uint32 sound = SOUND_SCREAM_1 + urand(0, 3);
        ...
        uint64 GetData64(uint32 id) const
        {
            ...
            {
                // GameObjects
            case DATA_NOTH_ENTER_GATE:
                return _nothGateEntranceGUID;
            case DATA_THADDIUS_GATE:
                return _thaddiusGateGUID;
            case DATA_THADDIUS_TESLA:
                return _thaddiusTeslaGUID;
            case DATA_THADDIUS_TESLA_TWO:
                return _thaddiusTeslaTwoGUID;
