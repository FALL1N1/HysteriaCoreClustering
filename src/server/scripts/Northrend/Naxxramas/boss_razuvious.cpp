...
public:
    ...
    struct boss_razuviousAI : public ScriptedAI
    {
        ...
        void EnterCombat(Unit *who)
        {
            ...
            me->StopMoving();
            events.ScheduleEvent(EVENT_SPELL_UNBALANCING_STRIKE, urand(5000, 8000));
...
// Still have to solve the issue around 'dismissing' the adds.
...
public:
    ...
    {
        boss_razuvious_minionAI(Creature* c) : ScriptedAI(c), _instance(c->GetInstanceScript())/*, bloodStrikeTimer(0)*/
        {
            c->LoadEquipment(1);
        }
        ...
        void EnterCombat(Unit *who)
        {
            ...
            if (Creature* cr = me->FindNearestCreature(NPC_RAZUVIOUS, 100.0f))
            {
                ...
                cr->AI()->DoZoneInCombat(nullptr, 250.0f);
            }
