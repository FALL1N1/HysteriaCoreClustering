			...
			void UpdateAI(uint32 diff)
			{
				...
				switch (uint32 eventId = events.ExecuteEvent())
				{
					...
					case EVENT_HADRONOX_MOVE3:
						...
						me->GetMotionMaster()->MoveCharge(hadronoxSteps[eventId-1].GetPositionX(), hadronoxSteps[eventId-1].GetPositionY(), hadronoxSteps[eventId-1].GetPositionZ(), 10.0f, 0, true);
						break;
