...
{
public:
    friend class ACE_Singleton<OpcodeTable, ACE_Null_Mutex>;
    ...
    void ValidateAndSetOpcode(bool isInValidRange, bool isNonZero, OpcodeHandler *opcodeHandler)
    {
        if (isInValidRange && isNonZero)
        {
            ...
            {
                sLog->outError("Tried to override handler of %s with %s (opcode %u)", _opcodes[opcodeHandler->opcode]->name, opcodeHandler->name, opcodeHandler->opcode);
        ...
        {
            sLog->outError("Tried to set handler for an invalid opcode %d", opcodeHandler->opcode);
            delete opcodeHandler;
        }
        else if (isInValidRange && !isNonZero)
        {
            sLog->outError("Opcode %s got value 0", opcodeHandler->name);
            delete opcodeHandler;
        }
        else
            sLog->outCrash("OpcodeTable: Something went horribly wrong");
    }
...
#define sOpcodeTable ACE_Singleton<OpcodeTable, ACE_Null_Mutex>::instance()
