...
public:
    ...
    void run()
    {
        ...
        while (!World::IsStopped())
        {
            ...
            else if (getMSTimeDiff(_lastChange, curtime) > (_delayTime + 30000))
            {
...
//    sLog->outString("%s (worldserver-daemon)", _FULLVERSION);
    ...
    uint16 psport = sConfigMgr->GetIntDefault("PoolServerPort", 8200);
    ...
    } else sLog->outString("Network is live, node open to connections. %s:%u", bindIp.c_str(), worldPort);
    if (sPoolSocketMgr->StartNetwork(psport, bindIp.c_str()) == -1)
    {
        sLog->outString("Failed to start cluster network, please check error log.");
        World::StopNow(ERROR_EXIT_CODE);
        // go down and shutdown the server
    }
    else sLog->outString("Connected to cluster, %s:%u", bindIp.c_str(), psport);
...
{
    if (sConfigMgr->GetIntDefault("CoreType", 0) > NODE_TYPE_MASTER)
        return;
